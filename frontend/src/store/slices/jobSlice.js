...
export const fetchSingleJob = (jobId) => async (dispatch) => {
  try {
    ...
    dispatch(jobSlice.actions.clearAllErrors()
    //   {
    //   type: "jobs/setSingleJob",
    //   payload: response.data.job,
    // }
  );
  } catch (error) {
    dispatch(
      jobSlice.actions.failureForAllJobs(error.response?.data?.message || "Failed to fetch single job"
      )
    );
  }
};
...
export const postJob = (FormData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post("http://localhost:4000/api/v1/job/post",
      FormData,
      {
    ...
    dispatch(jobSlice.actions.failureForPostJob(error.response?.data?.message||"Job post failed"));
  }
};
